...
## 모두의 페스타 적용 규칙 — 원본 분석보다 우선

이 문서의 Airbnb 분석은 구조 참고용이며 브랜드를 그대로 재현하지 않는다. 모두의 페스타는 연인과 가족을 위한 '함께 떠나는 주말'을 지향한다. 가족은 아이 동반과 부모님 동반 나들이를 모두 포함한다.

- 색상: 선명한 탠저린 primary `#ff7629`, active `#ff6414`, disabled `#ffdbbc`, canvas `#fffdf8`, ink `#30271f`, muted `#75685b`, hairline `#ecdfcf`, soft surface `#fff4e5`, strong surface `#ffe8cd`. 작은 강조 글자는 `#b6400b`, 주황색 버튼 글자는 어두운 ink로 대비를 확보한다. 본문의 Rausch 토큰 대신 사용한다.
- 밝고 따뜻하며 비비드한 인상을 유지한다. 탁한 자두색·회보라색은 사용하지 않는다. 밝은 핑크 워드마크와 중앙 밑줄 탭 조합도 피한다.
- 검색은 그림자 있는 알약 대신 얇은 선의 14px 필터 바. 모바일도 둥근 사각형이며 원형 검색 버튼 대신 펼침 표시를 쓴다.
- 제목은 28–32px로, 작은 '함께 떠나는 주말' 라벨과 함께 표현한다. 큰 광고 히어로는 만들지 않는다.
- 데이트 전용 표현 대신 축제·나들이·함께라는 표현을 사용한다. 가족 친화 여부, 운영 요일, 편의시설은 공식 근거 없이 추정하지 않는다. 기간 길이 분류는 실제 행사 유형 분류와 구별한다.
